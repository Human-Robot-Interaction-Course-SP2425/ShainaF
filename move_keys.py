"""
Shaina Freeland: Lab 1 code for key binds using the command line.
...
# CLI prompt
prompt = "(f)ear / (a)anger / (r)set / (q)uit: "
...
def handle_input(robot, cmd, args=[]):
    ...
    idle_sep = '='
    # bind to 'a' key and play sequence "anger_glare" 
    if cmd == 'a':
        seq = 'anger_glare'
    ...
    #Bind to 'f' key and play sequence "fear_look"
    elif cmd == 'f':
        seq = 'fear_look'
        if seq in robot.seq_list:
            # print("Playing sequence: %s"%(args[0]))
